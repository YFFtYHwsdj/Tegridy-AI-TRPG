...
from src.models import NPC, AgentNote, Character, PowerTag, Status, StoryTag
...
class TestResolveTarget(unittest.TestCase):
    def setUp(self):
        ...
        self.scene.npcs[self.npc.npc_id] = self.npc

    def test_empty_target(self):
        result = EffectApplicator._resolve_target("", self.character, self.scene)
        self.assertIsNone(result)

    def test_keyword_self_cn(self):
        result = EffectApplicator._resolve_target("自身", self.character, self.scene)
        self.assertIs(result, self.character)

    def test_keyword_self_en(self):
        result = EffectApplicator._resolve_target("self", self.character, self.scene)
        self.assertIs(result, self.character)

    def test_exact_char_name(self):
        result = EffectApplicator._resolve_target("Kael", self.character, self.scene)
        self.assertIs(result, self.character)

    def test_exact_chal_name(self):
        result = EffectApplicator._resolve_target("Miko 与她的保镖", self.character, self.scene)
        self.assertIs(result, self.npc)
    ...
    def test_no_match(self):
        result = EffectApplicator._resolve_target("外星人", self.character, self.scene)
        self.assertIsNone(result)

    def test_short_no_match(self):
        result = EffectApplicator._resolve_target("Ka", self.character, self.scene)
        self.assertIsNone(result)

    def test_fuzzy_char_only(self):
        char = Character(name="Kael", description="")
        scene = SceneState()
        npc = NPC(name="Miko", description="")
        scene.npcs[npc.npc_id] = npc
        result = EffectApplicator._resolve_target("Kae", char, scene)
    ...
    def test_fuzzy_chal_only(self):
        ...
        npc = NPC(name="Miko", description="")
        scene.npcs[npc.npc_id] = npc
        result = EffectApplicator._resolve_target("Mik", char, scene)
        self.assertIs(result, npc)

    def test_fuzzy_ambiguous_resolved_to_char(self):
        char = Character(name="Kael", description="")
        scene = SceneState()
        npc = NPC(name="KaelM", description="")
        scene.npcs[npc.npc_id] = npc
        result = EffectApplicator._resolve_target("Kael", char, scene)
        self.assertIs(result, char)

    def test_fuzzy_ambiguous_resolved_to_chal(self):
        char = Character(name="KaelM", description="")
        scene = SceneState()
        npc = NPC(name="Kael", description="")
        scene.npcs[npc.npc_id] = npc
        result = EffectApplicator._resolve_target("Kael", char, scene)
        self.assertIs(result, npc)
    ...
    def test_fuzzy_ambiguous_unresolved(self):
        ...
        scene = SceneState()
        npc = NPC(name="KaelY", description="")
        scene.npcs[npc.npc_id] = npc
        result = EffectApplicator._resolve_target("Kael", char, scene)
        self.assertIsNone(result)
...
class TestApplyEffectList(unittest.TestCase):
    ...
    def test_inflict_status_new(self):
        ...
        errors = EffectApplicator._apply_effect_list(effects, self.character, self.scene)
        self.assertEqual(errors, [])
        self.assertIn("受伤", self.character.statuses)
        self.assertEqual(self.character.statuses["受伤"].current_tier, 2)

    def test_inflict_status_on_npc(self):
        effects = [
            {
                "operation": "inflict_status",
                "target": "Miko",
        ...
        errors = EffectApplicator._apply_effect_list(effects, self.character, self.scene)
        self.assertEqual(errors, [])
        self.assertIn("被说服", self.npc.statuses)
...
class TestApplyResults(unittest.TestCase):
    ...
    def test_apply_effects_and_consequences(self):
        ...
        errors = EffectApplicator.apply_results(outcome_note, self.character, self.scene)
        self.assertEqual(errors, [])
        self.assertIn("受伤", self.character.statuses)
        self.assertEqual(self.character.statuses["受伤"].current_tier, 2)
        self.assertIn("被激怒", self.npc.statuses)
        self.assertEqual(self.npc.statuses["被激怒"].current_tier, 1)

    def test_no_character(self):
        errors = EffectApplicator.apply_results(None, None, self.scene)
        self.assertEqual(errors, [])

    def test_none_notes(self):
        errors = EffectApplicator.apply_results(None, self.character, self.scene)
        self.assertEqual(errors, [])
