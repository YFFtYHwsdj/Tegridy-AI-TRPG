...
class TestMDCleaner(unittest.TestCase):
    ...
    def test_save_nodes_to_file(self, mock_makedirs):
        nodes = [
            ...
            CleanedNode(
                level=1, title="Chapter 1", content="Content 1 continuation"
            ),  # duplicate title
        ...
        mock_makedirs.assert_called_once_with(
            os.path.dirname(os.path.abspath("output/test.md")), exist_ok=True
        )
        handle = m_open()
    ...
    def test_clean_markdown_nodes_success(self):
        ...
        mock_response = {
            ...
            "nodes": [{"level": 1, "title": "Cleaned", "content": "Content"}],
        }
    ...
    @patch(
        "sys.argv",
        ["md_cleaner.py", "-i", "input.md", "-o", "output.md", "--limit", "1", "--debug"],
    )
    @patch("src.tools.md_cleaner.os.getenv")
    def test_main(
        self, mock_getenv, mock_llm_class, mock_save, mock_clean, mock_parse, mock_get_raw
    ):
        mock_getenv.side_effect = lambda k, d=None: (
            "dummy" if k in ["DEEPSEEK_API_KEY", "DEEPSEEK_BASE_URL"] else d
        )
    ...
    def test_main_with_hierarchy_and_debug(
        self,
        mock_remove,
        mock_exists,
        mock_getenv,
        ...
        mock_parse.return_value = [
            ...
            RawNode(
                level=1, title="C2", raw_content="B" * 600
            ),  # > 500 chars to hit next_text truncation
